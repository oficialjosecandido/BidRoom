 ...
 * When `token` is null (no recipient record), omit the link so we never ship a dead `#` href.
 */
function buildPreferencesFooter({ token, type, texts }) {
  if (!token) return '';
  const href = `${frontendBaseUrl()}/email-preferences?token=${encodeURIComponent(token)}&type=${encodeURIComponent(type)}`;
  return `
